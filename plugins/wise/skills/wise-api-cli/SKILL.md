...
Use this skill when the task is about Wise API automation and the `wise` CLI is the preferred execution path.
...
4. Run `wise --help` to confirm the CLI is available. If it is not installed, use `npx -y @brahimhamichan/wise-cli --help` or ask before installing it globally.
5. Use `wise api` as the fallback for endpoints or query shapes that do not have a dedicated command yet.
...
```bash
wise auth status
npx -y @brahimhamichan/wise-cli auth status
